...
export default function DrawdownChart({ data = [], height = 220 }: DrawdownChartProps) {
  const { chartData, modes } = useMemo(() => toModeSeries(data), [data])

  if (chartData.length === 0) {
    return <div className="flex items-center justify-center text-sm text-gray-400" style={{ height }}>No drawdown data</div>
  }
...
function toModeSeries(data: BacktestEquityPoint[]): { chartData: Array<Record<string, number | string>>; modes: BacktestStrategyMode[] } {
  ...
  for (const point of data) {
    const bucket = byTimestamp.get(point.timestamp) ?? { timestamp: point.timestamp.replace('T', ' ').slice(5, 16) }
    bucket[point.strategy_mode] = point.drawdown_pct
