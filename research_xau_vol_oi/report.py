...
from research_xau_vol_oi.cme_overlap_backtest_lab import (
    ...
    cme_overlap_backtest_report_lines,
...
from research_xau_vol_oi.xau_trade_quality_failure_diagnostic import (
    ...
    xau_trade_quality_failure_diagnostic_report_lines,
)
from research_xau_vol_oi.xau_trade_quality_usage_guide import (
    ...
    xau_trade_quality_usage_guide_report_lines,
)
...
def run_pipeline(
    *,
    ...
    xau_trade_quality_failure_diagnostic = run_xau_trade_quality_failure_diagnostic(
        output_dir=output_root,
    )
    xau_trade_quality_usage_guide = run_xau_trade_quality_usage_guide(
        output_dir=output_root,
    )
    ...
    cme_wall_strategy_realism_audit = run_cme_wall_strategy_realism_audit(
        output_dir=output_root,
    )
    ...
    cme_wall_entry_filter_audit = run_cme_wall_entry_filter_audit(
        output_dir=output_root,
    )
    ...
    write_research_report(
        ...
        cme_wall_entry_filter_audit=cme_wall_entry_filter_audit,
    ...
    return {
        ...
        "cme_overlap_backtest_date_audit": output_root / "cme_overlap_backtest_date_audit.csv",
        "cme_overlap_candidate_source_report": output_root / "cme_overlap_candidate_source_report.csv",
        "cme_overlap_trade_candidates": output_root / "cme_overlap_trade_candidates.csv",
        "cme_overlap_filter_backtest": output_root / "cme_overlap_filter_backtest.csv",
        ...
        "xau_trade_quality_score_components": output_root / "xau_trade_quality_score_components.csv",
        "xau_trade_quality_score": output_root / "xau_trade_quality_score.csv",
        ...
        "xau_score_bucket_inversion_audit": output_root / "xau_score_bucket_inversion_audit.csv",
        "xau_score_component_failure_audit": output_root / "xau_score_component_failure_audit.csv",
        "xau_score_forward_join_audit": output_root / "xau_score_forward_join_audit.csv",
        "xau_score_regime_breakdown": output_root / "xau_score_regime_breakdown.csv",
        "xau_score_failure_decision": output_root / "xau_score_failure_decision.csv",
        "xau_trade_quality_usage_guide": output_root / "xau_trade_quality_usage_guide.md",
        "xau_trade_quality_component_guide": output_root / "xau_trade_quality_component_guide.csv",
        ...
        "cme_wall_strategy_ledger_reconciliation": output_root
        / "cme_wall_strategy_ledger_reconciliation.csv",
        "cme_wall_strategy_trade_realism_audit": output_root
        / "cme_wall_strategy_trade_realism_audit.csv",
        "cme_wall_strategy_gross_loss_sanity": output_root
        / "cme_wall_strategy_gross_loss_sanity.csv",
        "cme_wall_strategy_independent_event_performance": output_root
        / "cme_wall_strategy_independent_event_performance.csv",
        "cme_wall_strategy_corrected_quality_grade": output_root
        / "cme_wall_strategy_corrected_quality_grade.csv",
        "cme_wall_strategy_entry_exit_rewrite_plan": output_root
...
def run_dukascopy_only_pipeline(
    *,
        ...
        "\n".join(
            [
                ...
                *xau_trade_quality_forward_monitor_report_lines(trade_quality_forward_monitor),
                "",
                *xau_trade_quality_failure_diagnostic_report_lines(trade_quality_failure_diagnostic),
                "",
                ...
                *cme_wall_strategy_performance_report_lines(
                ...
                "",
                *cme_wall_strategy_realism_audit_report_lines(
                    cme_wall_strategy_realism_audit,
                ),
                "",
                *cme_wall_realistic_backtest_v1_report_lines(
    ...
    return {
        ...
        **{
            name: path
            for name, path in trade_quality_failure_diagnostic.paths.items()
            if name.endswith("_csv") or name.endswith("_md")
        },
        **{
            name: path
            for name, path in trade_quality_usage_guide.paths.items()
            if name.endswith("_csv") or name.endswith("_md")
        },
        **{
            name: path
        ...
        **{
            name: path
            for name, path in cme_wall_strategy_realism_audit.paths.items()
            if name.endswith("_csv") or name.endswith("_md")
        },
        **{
            name: path
        ...
        **{
            name: path
            for name, path in cme_wall_entry_filter_audit.paths.items()
            if name.endswith("_csv") or name.endswith("_md")
        },
    }
...
def write_research_report(
    ...
    *,
    ...
    cme_wall_entry_filter_audit: CmeWallEntryFilterAuditResult | None = None,
    ...
    text = [
        ...
        *xau_trade_quality_forward_monitor_report_lines(xau_trade_quality_forward_monitor),
        "",
        *xau_trade_quality_failure_diagnostic_report_lines(xau_trade_quality_failure_diagnostic),
        "",
        ...
        *cme_wall_realistic_loss_decomposition_report_lines(
        ...
        "",
        *cme_wall_entry_filter_audit_report_lines(cme_wall_entry_filter_audit),
        "",
