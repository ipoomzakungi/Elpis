...
from src.models.data_sources import DataSourceBootstrapRequest
...
async def test_public_bootstrap_with_mocked_binance_and_yahoo_generates_readiness(
    ...
    assert result.bootstrap_run_id.startswith("bootstrap_")
    assert {summary.provider_type for summary in result.asset_summaries} == {
        "binance_public",
        "yahoo_finance",
    }
    ...
    assert result.preflight_result.status == "partial"
    ...
    yahoo = next(
        ...
        if summary.provider_type == "yahoo_finance"
    )
    assert "open_interest" in yahoo.unsupported_capabilities
    assert any("OHLCV-only" in note for note in yahoo.limitations)


@pytest.mark.asyncio()
async def test_bootstrap_to_preflight_readiness_transition_for_generated_files(
    isolated_data_paths,
):
    service = PublicDataBootstrapService(
        binance_client=_MockBinanceClient(),
    ...
    result = await service.run(
        DataSourceBootstrapRequest(
            ...
            binance_timeframes=["1h"],
            yahoo_symbols=["GLD"],
            yahoo_timeframes=["1d"],
            include_xau_local_instructions=False,
            research_only_acknowledged=True,
        )
    )
    ...
    assert preflight.crypto_results[0].asset == "ETHUSDT"
    assert preflight.crypto_results[0].status == "ready"
    assert preflight.proxy_results[0].asset == "GLD"
    assert preflight.proxy_results[0].status == "ready"
    assert any("gold OHLCV proxies only" in note for note in preflight.proxy_results[0].limitations)


class _MockBinanceClient:
    async def download_ohlcv(self, symbol: str, interval: str, days: int) -> pl.DataFrame:
        return _ohlcv_frame(symbol=symbol, timeframe=interval, rows=48)

    async def download_open_interest(self, symbol: str, period: str, days: int) -> pl.DataFrame:
        base = _ohlcv_frame(symbol=symbol, timeframe=period, rows=12)
        return pl.DataFrame(
            {
...
class _MockYahooClient:
    async def download_ohlcv(self, symbol: str, timeframe: str, days: int) -> pl.DataFrame:
        return _ohlcv_frame(symbol=symbol, timeframe=timeframe, rows=42).with_columns(
            [
