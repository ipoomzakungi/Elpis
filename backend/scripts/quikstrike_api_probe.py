"""Run a local-only QuikStrike HTTP/API login probe.
...
from src.quikstrike.api_probe import (
    ...
    QuikStrikeApiProbeError,
    default_api_probe_path,
    run_quikstrike_api_probe,
    write_api_probe_report,
)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Probe QuikStrike HTTP login and sanitized API candidates.",
        parents=[env_parser],
    )
    parser.add_argument(
        "--start-url",
    ...
    output_path = (
        ...
        if args.output_path
        else default_api_probe_path(repo_backend_dir=Path(__file__).resolve().parents[1])
    )
    ...
    try:
        ...
        write_api_probe_report(report, output_path)
    except QuikStrikeApiProbeError as exc:
        ...
        json.dumps(
            {
                ...
                "authenticated_page_reachable": report["authenticated_page_reachable"],
                "login_step_count": report["login_step_count"],
                "request_step_count": report["request_step_count"],
                "api_candidate_count": report["api_candidate_count"],
...
def _load_env_file(path: Path) -> dict[str, str]:
    ...
    for line_number, raw_line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if normalized_key not in ALLOWED_ENV_FILE_KEYS:
            raise ValueError(
                f"{path} contains unsupported QuikStrike API env key: {normalized_key}"
            )
