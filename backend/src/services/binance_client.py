...
class BinanceClient:
    ...
    async def get_klines(
        ...
        response = await self.client.get(f"{self.base_url}/fapi/v1/klines", params=params)
    ...
    async def get_funding_rate(
        ...
        response = await self.client.get(f"{self.base_url}/fapi/v1/fundingRate", params=params)
        response.raise_for_status()
        return response.json()
    ...
    async def download_open_interest(
        ...
        df = pl.DataFrame(
            {
                "timestamp": [datetime.fromtimestamp(int(d["timestamp"]) / 1000) for d in all_data],
    ...
    async def download_funding_rate(
        ...
        df = pl.DataFrame(
            {
                "timestamp": [datetime.fromtimestamp(int(d["fundingTime"]) / 1000) for d in all_data],
                "symbol": [d["symbol"] for d in all_data],
