...
class RegimeClassifier:
    ...
    def classify_bar(
        ...
        values = [price, range_high, range_low, range_mid, volume_ratio]
        if oi_change_pct is not None:
            values.append(oi_change_pct)
        if not all(isfinite(value) for value in values):
            return RegimeType.AVOID, 0.0, "Invalid numeric feature value"
        ...
        # Default to AVOID
        oi_text = f"{oi_change_pct:.1f}%" if oi_change_pct is not None else "N/A"
        return (
            RegimeType.AVOID,
            0.5,
            (f"Conflicting signals: price position {price_position:.1%}, OI change {oi_text}"),
        )

    def classify_dataframe(self, df: pl.DataFrame) -> pl.DataFrame:
        """Classify all bars in a DataFrame."""
        required_columns = {"close", "range_high", "range_low", "range_mid", "volume_ratio"}
        missing = sorted(required_columns.difference(df.columns))
        if missing:
            raise ValueError(f"Missing columns for regime classification: {', '.join(missing)}")
        ...
        logger.info("Classified %s rows into regimes", len(regimes))
