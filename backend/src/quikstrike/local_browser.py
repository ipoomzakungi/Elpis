"""Local browser adapter boundary for future user-controlled QuikStrike extraction.

This module intentionally does not connect to a browser, replay endpoints, or persist
session material. It only validates sanitized shape inputs for later integration.
"""
...
from src.models.quikstrike import ensure_no_forbidden_quikstrike_content

SUPPORTED_SURFACE = "QUIKOPTIONS VOL2VOL"
SUPPORTED_PRODUCT_CODE = "OG|GC"


class LocalBrowserExtractionNotImplementedError(RuntimeError):
    """Raised when real browser extraction is requested in the skeleton slice."""


@dataclass(frozen=True)
class LocalBrowserReadiness:
    ready: bool
    status: str
    message: str
...
class QuikStrikeLocalBrowserAdapter:
    ...
    def readiness(
        ...
        *,
        ...
        surface_ready = "quikoptions" in normalized_surface and "vol2vol" in normalized_surface
        ...
        return LocalBrowserReadiness(
            ...
            message=(
                "Open QuikStrike manually, sign in manually, navigate to "
                "QUIKOPTIONS VOL2VOL, and select Gold (OG|GC)."
            ),
            limitations=_limitations(),
        )

    def collect_current_page_payload(self) -> None:
        """Placeholder only; real browser collection is intentionally not implemented."""

        raise LocalBrowserExtractionNotImplementedError(
...
def _limitations() -> list[str]:
    return [
        ...
        "or private full URLs are accepted or persisted.",
        "No real browser automation, endpoint replay, RPA, OCR, or network calls are performed.",
